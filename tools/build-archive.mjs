...
const additional=read('data/additional-records.json');
Object.assign(research.sources,additional.sources);
const allIds=new Set(research.chats.flatMap(c=>c.messages.map(m=>m.id)));
for(const {chatId,message} of additional.records){
 const chat=research.chats.find(c=>c.id===chatId);
 if(!chat||allIds.has(message.id))throw Error('Invalid additional record: '+message.id);
 allIds.add(message.id);
 // Insert among the existing dated records, preserving their relative order.
 const key=m=>m.t||m.d||'9999';
 const position=chat.messages.findIndex(m=>key(m)>key(message));
 chat.messages.splice(position<0?chat.messages.length:position,0,message);
}
...
out.archiveMeta.additionalRecords=additional.records.length;
out.archiveMeta.additionalSnapshot='data/additional-records.json';
// Threads: fatos datados da pesquisa e cobertura que não sustenta registro (só manchete, data e link).
out.timeline=(research.timeline||[]).map(e=>({id:e.id,date:e.date,title:e.title||'',text:e.text,sources:(e.src||[]).map(sid).filter(id=>sources[id])})).filter(e=>e.sources.length);
out.press=(research.press||[]).filter(s=>!sources[sid(s.id)]).map(s=>({...s,id:sid(s.id)}));
