...
test('perícia começa ativa e citações ficam limitadas por fonte',()=>{assert.equal(ctx.window.ForensicManager.isForensicModeActive,true);const words={};for(const c of d.contacts)for(const m of c.messages)if(m.editorialType==='quote'&&!m.researchId){const id=m.sources[0];words[id]=(words[id]||0)+m.text.split(/\s+/).length;}for(const n of Object.values(words))assert(n<=25);});
...
test('Moraes tem contraponto e nenhuma resposta literal atribuída a ele',()=>{const c=d.contacts.find(c=>c.id==='stf_alexandre');assert(/contesta|nega/.test(c.contextSummary));assert(c.messages.every(m=>m.editorialType!=='quote'||m.speaker==='Daniel Vorcaro'));});
...
test('encaminhamento não cria conversa direta com Gonet',()=>{assert(!d.contacts.some(c=>c.id==='gonet'));const c=d.contacts.find(c=>c.id==='ciro_soares');assert(c.messages.some(m=>m.researchId&&/encaminh/i.test(m.text+' '+m.context)));assert(c.responseSources.includes('dos_gonet_resp'));});
...
test('fotos e galerias têm procedência e acesso à imagem completa',()=>{
 ...
 assert.deepEqual(Array.from(records,m=>m.id).sort(),['ds-cr3','ds-nr1','ds-nr2','mf4','r-A2-m23','r-A2-m37','r-A2-m70']);
 let count=0;
